...
while True:
    ...
    # texto na tela da posição X central do objeto
    cv2.putText(frame, str(center), center, cv2.FONT_HERSHEY_SIMPLEX, 0.7, (0, 0, 255), 2)
    # texto na tela da posição Y central do objeto
    cv2.imshow("frame", frame)
    cv2.imshow("mask", mask)
    cv2.imshow("res", res)

    if center == None:                  # quando não encontrar nenhum objeto, manda os servos para uma posição
        Xmotor = 90
        Ymotor = 90

    if x > 0 and y > 0:
        move_servo1(a=Ymotor)          # movimenta o servo 1 de acordo com a posição na tela
        move_servo2(b=Xmotor)           # movimenta o servo 2 de acordo com a posição na tela
